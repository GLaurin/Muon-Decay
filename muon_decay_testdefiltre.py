...
seuil = 0.005
for i in range (10):
    data = np.loadtxt("C:\\Users\\sasch\\Desktop\\Stage été 2021\\data_muons_0602\\Deux pics\\testdefiltre_"+str(i+1)+".txt")
    x = data[:,0]
    y = data[:,1]
    
    l_min       = 4                         # Largeur minimale d'integration
    l_max       = 10                        # Largeur maximale d'integration
    ...
    '''
    plt.close()
    plt.savefig("C:\\Users\\sasch\\Desktop\\Stage été 2021\\codes\\Muon-Decay\\Desintegrations\\testdefiltre_"+str(i+1))