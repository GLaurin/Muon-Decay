...
plt.plot(t_lin, MuonCount(t_lin,*popt),'k', label="Curve_fit:" + r"$\tau$" + f"= {popt[1]:.3e} $\pm$ {np.sqrt(np.diag(pcov))[1]:.2e}")
plt.legend()
plt.savefig(args.folder+"\\Decays\\Histogramme_désintégrations_seuil"+str(args.seuil)[2:]+"_dpmin"+str(args.dp_min))
plt.close()
